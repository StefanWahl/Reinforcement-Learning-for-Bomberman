...
##########################################################################
# Stuff we load
from agent_code.agent_agent_3.Experience_Buffer import Experience_Buffer
from agent_code.agent_agent_3.Gradient_Boosting_Model import GBM_Model
from agent_code.agent_agent_3.Hyperparameters import *
##########################################################################
...
def setup_training(self):
    ...
    self.buffer = Experience_Buffer(D=FEATURE_DIM, gamma=GAMMA, N_max=N_MAX)
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if e.BOMB_DROPPED in events:
        ...
        if pos[0] % 2 != 0: close_environmentX = new_game_state["field"][pos[0],max(0,pos[1]-2):min(16,pos[1]+2)]
        else:close_environmentX = np.array([])

        if  pos[1] % 2 != 0: close_environmentY = new_game_state["field"][max(0,pos[0]-2):min(16,pos[0]+2),pos[1]]
        ...
        for other in old_game_state["others"]:
            if pos[0] % 2 != 0 and other[3][0] == pos[0] and abs(other[3][0] - pos[0]) < 2: 
                events.append(AIMED_BOMB_OPPONENT)
                #events.append(AIMED_BOMB)

            elif pos[1] % 2 != 0 and other[3][1] == pos[0] and abs(other[3][1] - pos[1]) < 2: 
                events.append(AIMED_BOMB_OPPONENT)
                #events.append(AIMED_BOMB)
