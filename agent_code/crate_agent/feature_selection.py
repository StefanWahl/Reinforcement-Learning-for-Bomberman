...
def path_finder(game_map,start,target,blocked):
    ...
    while len(q) > 0:
        ...
        for n in [(pos[0]+1,pos[1]),(pos[0]-1,pos[1]),(pos[0],pos[1]+1),(pos[0],pos[1]-1)]:
            ...
            if (game_map[n[0],n[1]] in blocked) or (n in parents.keys()): continue
...
def danger_close(pos,times,bombs,explosion_map):
    ...
    #if there is an explosion, there is danger
    ...
    elif  (0 in times) or (1 in times) or (2 in times) or (3 in times) or (4 in times):
...
def features_V2(gam_state):
    ...
    bombing_score_coins = np.zeros(len(coin_pos))
    ...
    #######################################################################################################################
    #Other players // Dummy Implementierung
    #######################################################################################################################
    other_pos = np.array([list(coin) for coin in gam_state["others"]])
    ...
    for i in range(len(other_pos)):
        ...
        #Is reachable
            ...
            if other_pos[i][0] == own_position[0]:
                if other_pos[i][0] % 2 != 0 and abs(own_position[0] - other_pos[i][0]) < 4: bombable.append(1)
                else: bombable.append(0)

            elif other_pos[i][1] == own_position[1]:
                if other_pos[i][1] % 2 != 0 and abs(own_position[1] - other_pos[i][1]) < 4: bombable.append(1)
                else: bombable.append(0)
