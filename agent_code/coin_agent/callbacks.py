...
##########################################################################
# Stuff we load
from agent_code.coin_agent.feature_selection import coins_simple as get_features
...
def act(self, game_state: dict):
    ...
    #Training mode: Use an epsilon greedy policy
    if self.train:
        ...
        if Q < epsilon:
            action_index = np.random.choice(np.arange(6), p = [1/4,1/4,1/4,1/4,0,0])
            self.rand_act += 1
            self.tot_act += 1
        else:
            p = self.model(X)
            action_index = np.argmax(p[:4])
            self.tot_act += 1

    #Evaluation mode: Just use the trained policy
    else:
        p = self.model(X)
        action_index = np.argmax(p[:4])
